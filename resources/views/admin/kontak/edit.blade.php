...
<div class="card">
    <div class="card-header">
        <h5>Edit Kontak & Media Sosial</h5>
    </div>
    <div class="card-body">
        <form action="{{ route('admin.kontak.update', $kontak->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select name="tipe" class="form-control" required>
                    @foreach($tipeList as $key => $label)
                        <option value="{{ $key }}" {{ old('tipe', $kontak->tipe) == $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @error('tipe')
            ...
            <div class="mb-3">
                ...
                <input type="text" name="judul" class="form-control" value="{{ old('judul', $kontak->judul) }}" placeholder="Contoh: Nomor WhatsApp Utama, Alamat Kantor, Instagram" required>
            ...
            <div class="mb-3">
                ...
                <textarea name="nilai" class="form-control" rows="4" placeholder="Masukkan detail kontak (alamat lengkap, nomor hp, email, username, atau jam buka)" required>{{ old('nilai', $kontak->nilai) }}</textarea>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="nilai_tambahan" class="form-control" value="{{ old('nilai_tambahan', $kontak->nilai_tambahan) }}" placeholder="Contoh: (Zen M. Siboro - Co-Founder) atau (Kecamatan Simanindo)">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="icon" class="form-control" value="{{ old('icon', $kontak->icon) }}" placeholder="Contoh: fas fa-phone-alt, fab fa-whatsapp, fas fa-envelope, fab fa-facebook-f, far fa-clock">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="tautan" class="form-control" value="{{ old('tautan', $kontak->tautan) }}" placeholder="Contoh: https://wa.me/6285362259937 atau mailto:admin@email.com atau link profil sosmed">
            ...
            <div class="mb-3">
                <label class="d-flex align-items-center">
                    <input type="checkbox" name="status" value="1" {{ old('status', $kontak->status) ? 'checked' : '' }} style="width: 20px; height: 20px; margin-right: 8px;">
                    <span>Aktifkan Kontak ini agar muncul di halaman publik</span>
                </label>
            </div>

            <button type="submit" class="btn btn-primary" style="background: linear-gradient(135deg, #10b981, #34d399) !important; border:none;">Update Kontak</button>
